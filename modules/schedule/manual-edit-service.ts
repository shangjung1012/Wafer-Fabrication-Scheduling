...
async function getAffectedOrderTypes(
  db: PrismaClient,
  ...
  const orderIds = moves.map((m) => m.orderId).filter(Boolean) as string[];
  ...
  for (const a of assignments) types.add(a.order.type);
  for (const o of orders) types.add(o.type);
