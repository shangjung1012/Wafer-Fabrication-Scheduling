...
import { PrismaClient, OrderStatus, AssignmentStatus, FactoryStatus, UserRole } from "../lib/generated/prisma/client";
import { prepareSchedulingData, _applyScheduleTransaction } from "../modules/schedule/core";
...
const BM_EXPECTED = { scheduled: 23_023, failed: 977, successRate: "95.9", capacityUtil: "81.2" };
...
async function seedBenchmarkData(operatorId: string): Promise<{ factoryIds: string[] }> {
  ...
  for (let i = 0; i < BM_NUM_FACTORIES; i++) {
    const f = await prisma.factory.create({
      data: { productionType: BENCHMARK_TYPE, maxCapacity: BM_CAPACITY, status: FactoryStatus.ACTIVE },
    });
    factoryIds.push(f.id);
  }

  // Full capacity — no pre-deduction for fixed orders, matching benchmark.ts behaviour
  const capBuf: { factoryId: string; date: Date; maxCapacity: number; curCapacity: number }[] = [];
  for (const fid of factoryIds) {
    for (let d = 0; d < BM_DAYS; d++) {
      capBuf.push({ factoryId: fid, date: addDays(TODAY, d), maxCapacity: BM_CAPACITY, curCapacity: BM_CAPACITY });
    }
  }
  ...
  for (let i = 0; i < mutableBuf.length; i += 500) {
    await prisma.order.createMany({ data: mutableBuf.slice(i, i + 500) as never[] });
  }
  await prisma.order.createMany({ data: fixedBuf as never[] });
  await prisma.orderAssignment.createMany({ data: fixedAsgBuf as never[] });

  console.log(`  Done: ${BM_NUM_ORDERS} mutable + ${BM_NUM_FIXED} fixed, ${factoryIds.length} factories.\n`);
  return { factoryIds };
}
...
async function main() {
  ...
  const prepared = await prepareSchedulingData(BENCHMARK_TYPE, { ...bmConfig }, TODAY, true);
  console.log(`  prepareSchedulingData: ${fmt(performance.now() - t0read)}  |  Orders: ${prepared.orders.length}  |  Capacities: ${prepared.capacities.length}`);

  // ── Strategy (in-memory) ─────────────────────────────────────────────────
  const t0strat = performance.now();
  const stratResult = greedyBestFitStrategy.execute(prepared.orders, prepared.factories, prepared.capacities, bmConfig, TODAY);
  const stratMs = performance.now() - t0strat;

  const inMemScheduled = stratResult.processedOrders.filter(o => o.status === OrderStatus.SCHEDULED && !o.isFixed).length;
  const inMemFailed    = stratResult.processedOrders.filter(o => o.status === OrderStatus.FAILED).length;
  console.log(`  Strategy:             ${fmt(stratMs)}  |  Scheduled: ${inMemScheduled}  |  Failed: ${inMemFailed}`);

  // ── _applyScheduleTransaction ────────────────────────────────────────────
  const t0write = performance.now();
  try {
    await withScheduleLock(BENCHMARK_TYPE, async () => {
      await _applyScheduleTransaction(BENCHMARK_TYPE, bmConfig, stratResult, operator.id);
    });
  } catch (e) {
  ...
  const dbOrders = await prisma.order.findMany({
    ...
    select: {
      id: true, status: true, quantity: true, dueDate: true, isFixed: true, isPrioritized: true,
      assignments: { select: { factoryId: true, productionDate: true, assignedQuantity: true } },
    },
  });
  ...
  if (!negCap) { invPassed++; console.log("  ✅ [1/7] Capacity Non-Negative"); }
  else          { console.log(`  ❌ [1/7] Capacity Non-Negative — curCapacity=${negCap.curCapacity}`); }

  const overCap = dbCaps.find(c => c.curCapacity > c.maxCapacity);
  if (!overCap) { invPassed++; console.log("  ✅ [2/7] Capacity Not Exceeding Max"); }
  else          { console.log(`  ❌ [2/7] Capacity Not Exceeding Max — ${overCap.curCapacity} > ${overCap.maxCapacity}`); }

  let qtyFail: string | null = null;
  for (const o of dbOrders.filter(o => o.status === OrderStatus.SCHEDULED && !o.isFixed)) {
    const total = o.assignments.reduce((s, a) => s + a.assignedQuantity, 0);
    if (total !== o.quantity) { qtyFail = `${o.id}: expected ${o.quantity} got ${total}`; break; }
  }
  if (!qtyFail) { invPassed++; console.log("  ✅ [3/7] Quantity Conservation"); }
  else          { console.log(`  ❌ [3/7] Quantity Conservation — ${qtyFail}`); }
  ...
  if (windowFail === 0) { invPassed++; console.log("  ✅ [4/7] Time Window Compliance"); }
  else                  { console.log(`  ❌ [4/7] Time Window Compliance — ${windowFail} violations`); }

  const failedWithAsgn = dbOrders.filter(o => o.status === OrderStatus.FAILED && o.assignments.length > 0);
  if (failedWithAsgn.length === 0) { invPassed++; console.log("  ✅ [5/7] Rollback Completeness"); }
  else                             { console.log(`  ❌ [5/7] Rollback Completeness — ${failedWithAsgn.length} FAILED orders have assignments`); }

  const fixedNotScheduled = dbOrders.filter(o => o.isFixed && o.status !== OrderStatus.SCHEDULED);
  if (fixedNotScheduled.length === 0) { invPassed++; console.log("  ✅ [6/7] Immutable Order Protection"); }
  else                                { console.log(`  ❌ [6/7] Immutable Order Protection — ${fixedNotScheduled.length} fixed orders changed status`); }

  let seenNonPrio = false;
  let prioViolation: string | null = null;
  for (const o of stratResult.processedOrders.filter(o => !o.isFixed)) {
    if (o.isPrioritized) { if (seenNonPrio) { prioViolation = o.id; break; } }
    else                 { seenNonPrio = true; }
  }
  if (!prioViolation) { invPassed++; console.log("  ✅ [7/7] Prioritized Processing Order"); }
  else                { console.log(`  ❌ [7/7] Prioritized Processing Order — order ${prioViolation}`); }

  console.log(`\n  Invariant Pass Rate: ${invPassed}/7`);

  // ── Metrics vs benchmark.md ──────────────────────────────────────────────
  const mutableInDB    = dbOrders.filter(o => !o.isFixed);
  const scheduledCount = mutableInDB.filter(o => o.status === OrderStatus.SCHEDULED).length;
  const failedCount    = mutableInDB.filter(o => o.status === OrderStatus.FAILED).length;
  const successRate    = (scheduledCount / BM_NUM_ORDERS * 100).toFixed(1);
  const totalAssigned  = dbOrders.flatMap(o => o.assignments).reduce((s, a) => s + a.assignedQuantity, 0);
  const utilization    = (totalAssigned / (BM_NUM_FACTORIES * BM_CAPACITY * (BM_DAYS - 1)) * 100).toFixed(1);
  ...
  console.log(chk("Success Rate", successRate + "%", BM_EXPECTED.successRate + "%"));
  console.log(chk("Capacity Util", utilization + "%", BM_EXPECTED.capacityUtil + "%"));
  ...
  const metricsOk = scheduledCount === BM_EXPECTED.scheduled && failedCount === BM_EXPECTED.failed;
  const allPassed = invPassed === 7 && metricsOk;

  if (allPassed) {
    console.log(`  ✅ All checks passed`);
    console.log(`     Invariants: 7/7  |  Scheduled: ${scheduledCount}/${BM_NUM_ORDERS}  |  Write: ${fmt(writeMs)}`);
    console.log();
    console.log("🎉 DB End-to-End Benchmark passed.");
  } else {
    console.log(`  ❌ Invariants: ${invPassed}/7  |  Metrics match: ${metricsOk}`);
    console.log();
